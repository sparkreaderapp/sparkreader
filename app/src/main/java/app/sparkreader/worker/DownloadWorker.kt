...
package app.sparkreader.worker
...
import app.sparkreader.common.readLaunchInfo
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_ACCESS_TOKEN
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_APP_TS
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_ERROR_MESSAGE
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_FILE_NAME
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_MODEL_DIR
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_RATE
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_RECEIVED_BYTES
import app.sparkreader.data.KEY_MODEL_DOWNLOAD_REMAINING_MS
import app.sparkreader.data.KEY_MODEL_EXTRA_DATA_DOWNLOAD_FILE_NAMES
import app.sparkreader.data.KEY_MODEL_EXTRA_DATA_URLS
import app.sparkreader.data.KEY_MODEL_IS_ZIP
import app.sparkreader.data.KEY_MODEL_NAME
import app.sparkreader.data.KEY_MODEL_START_UNZIPPING
import app.sparkreader.data.KEY_MODEL_TOTAL_BYTES
import app.sparkreader.data.KEY_MODEL_UNZIPPED_DIR
import app.sparkreader.data.KEY_MODEL_URL
import app.sparkreader.data.KEY_MODEL_VERSION
...
class DownloadWorker(context: Context, params: WorkerParameters) :
    ...
    return withContext(Dispatchers.IO) {
      if (fileUrl == null || fileName == null) {
        ...
        return@withContext try {
          ...
          setProgress(
            Data.Builder()
              .putLong(KEY_MODEL_DOWNLOAD_RECEIVED_BYTES, 0L)
              .putLong(KEY_MODEL_DOWNLOAD_RATE, 0L)
              .putLong(KEY_MODEL_DOWNLOAD_REMAINING_MS, 0L)
              .build()
          )
          ...
          for (file in allFiles) {
            ...
            // Variable to hold the actual connection to use for download
            ...
            if (
              ...
              // Range Not Satisfiable - the partial file is invalid
              Log.w(TAG, "HTTP 416: Range Not Satisfiable. Deleting partial file and starting over.")
              ...
              // Reconnect without Range header to start fresh
              ...
              if (activeConnection.responseCode != HttpURLConnection.HTTP_OK) {
                throw IOException("HTTP error code: ${activeConnection.responseCode}")
              }
              
              append = false // Overwrite the file
              Log.d(TAG, "Restarted download from beginning after 416 error")
            } else {
  ...
  private fun createForegroundInfo(progress: Int, modelName: String? = null): ForegroundInfo {
    ...
    // Create an Intent to open the app with a flag to navigate to model manager
    ...
    val notification =
      NotificationCompat.Builder(applicationContext, FOREGROUND_NOTIFICATION_CHANNEL_ID)
        ...
        .setProgress(0, 0, true) // Always show indeterminate progress
        .setContentIntent(pendingIntent) // Make notification clickable
        .build()
