...
import app.sparkreader.worker.DownloadWorker
...
class DefaultDownloadRepository(
      ...
      if (workInfo != null) {
        when (workInfo.state) {
          WorkInfo.State.RUNNING -> {
            ...
            if (!startUnzipping) {
              if (receivedBytes == 0L && downloadRate == 0L) {
                // Still connecting
                onStatusUpdated(
                  model,
                  ModelDownloadStatus(status = ModelDownloadStatusType.CONNECTING),
                )
              } else {
                onStatusUpdated(
                  model,
  ...
  override fun setNotificationPermissionLauncher(launcher: ActivityResultLauncher<String>?) {
    notificationPermissionLauncher = launcher
  }
  
  override fun requestNotificationPermission() {
    notificationPermissionLauncher?.let { launcher ->
      Log.d(TAG, "Requesting notification permission for downloads")
      launcher.launch(Manifest.permission.POST_NOTIFICATIONS)
    } ?: run {
      Log.d(TAG, "No notification permission launcher available")
    }
  }
  ...
  private fun sendNotification(title: String, text: String, modelName: String) {
    ...
    val channelName = "SparkReader download notification"
    ...
    with(NotificationManagerCompat.from(context)) {
      // Use a unique notification ID based on model name to avoid conflicts
      val notificationId = modelName.hashCode()
